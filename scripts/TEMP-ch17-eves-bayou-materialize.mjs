import { readFileSync, writeFileSync } from "node:fs";
import { execFileSync } from "node:child_process";

function read(path) { return readFileSync(path, "utf8"); }
function write(path, value) { writeFileSync(path, value); }
function replaceOnce(path, from, to) {
  const source = read(path);
  const first = source.indexOf(from);
  if (first < 0) throw new Error(`${path}: missing anchor ${from.slice(0, 120)}`);
  if (source.indexOf(from, first + from.length) >= 0) throw new Error(`${path}: non-unique anchor ${from.slice(0, 120)}`);
  write(path, source.slice(0, first) + to + source.slice(first + from.length));
}
function replaceAllLiteral(path, from, to, minCount = 1) {
  const source = read(path);
  const count = source.split(from).length - 1;
  if (count < minCount) throw new Error(`${path}: expected >=${minCount} ${from}, found ${count}`);
  write(path, source.split(from).join(to));
}

replaceOnce(
  "src/ui/data/filmScenarios.ts",
  'import { mergeChapterSeventeenPrincessMononokeExpansion } from "../../core/chapterSeventeenPrincessMononokeExpansion.js";\n',
  'import { mergeChapterSeventeenPrincessMononokeExpansion } from "../../core/chapterSeventeenPrincessMononokeExpansion.js";\nimport { mergeChapterSeventeenEvesBayouExpansion } from "../../core/chapterSeventeenEvesBayouExpansion.js";\n',
);
replaceOnce(
  "src/ui/data/filmScenarios.ts",
  "const chapterSeventeenPrincessMononokeScenarios = mergeChapterSeventeenPrincessMononokeExpansion(chapterSeventeenTitanicScenarios);\nconst modernCanonScenarios = mergeModernCanonExpansion(chapterSeventeenPrincessMononokeScenarios);",
  "const chapterSeventeenPrincessMononokeScenarios = mergeChapterSeventeenPrincessMononokeExpansion(chapterSeventeenTitanicScenarios);\nconst chapterSeventeenEvesBayouScenarios = mergeChapterSeventeenEvesBayouExpansion(chapterSeventeenPrincessMononokeScenarios);\nconst modernCanonScenarios = mergeModernCanonExpansion(chapterSeventeenEvesBayouScenarios);",
);
replaceOnce(
  "src/ui/data/filmScenarios.ts",
  "+manual_chapter_seventeen_princess_mononoke_expansion_2026+manual_modern_indie_asian_prize_expansion_2026",
  "+manual_chapter_seventeen_princess_mononoke_expansion_2026+manual_chapter_seventeen_eves_bayou_expansion_2026+manual_modern_indie_asian_prize_expansion_2026",
);

replaceOnce(
  "src/ui/data/scenarioFilmStudyMap.ts",
  'import { princessMononokeFilmHistoryProfile } from "./scenarioFilmStudyChapterSeventeenPrincessMononoke";\n',
  'import { princessMononokeFilmHistoryProfile } from "./scenarioFilmStudyChapterSeventeenPrincessMononoke";\nimport { evesBayouFilmHistoryProfile } from "./scenarioFilmStudyChapterSeventeenEvesBayou";\n',
);
replaceOnce(
  "src/ui/data/scenarioFilmStudyMap.ts",
  "  [princessMononokeFilmHistoryProfile.scenarioId]: princessMononokeFilmHistoryProfile,\n  scenario_the_machinist_2004:",
  "  [princessMononokeFilmHistoryProfile.scenarioId]: princessMononokeFilmHistoryProfile,\n  [evesBayouFilmHistoryProfile.scenarioId]: evesBayouFilmHistoryProfile,\n  scenario_the_machinist_2004:",
);

replaceOnce(
  "src/ui/data/scenarioProductionVerificationRegistry.ts",
  'import { princessMononokeProductionCaseVerification } from "./scenarioProductionVerificationPrincessMononoke";\n',
  'import { princessMononokeProductionCaseVerification } from "./scenarioProductionVerificationPrincessMononoke";\nimport { evesBayouProductionCaseVerification } from "./scenarioProductionVerificationEvesBayou";\n',
);
replaceOnce(
  "src/ui/data/scenarioProductionVerificationRegistry.ts",
  "  princessMononokeProductionCaseVerification,\n  lonelyVillaProductionCaseVerification,",
  "  princessMononokeProductionCaseVerification,\n  evesBayouProductionCaseVerification,\n  lonelyVillaProductionCaseVerification,",
);

replaceOnce("scripts/production-case-rest-audit.mjs", "const EXPECTED_PLAYABLE_SCENARIOS = 487;", "const EXPECTED_PLAYABLE_SCENARIOS = 488;");
replaceOnce("scripts/production-case-rest-audit.mjs", "const EXPECTED_VERIFIED_PRODUCTION_CASES = 487;", "const EXPECTED_VERIFIED_PRODUCTION_CASES = 488;");
replaceOnce(
  "scripts/production-case-rest-audit.mjs",
  '  "chapterSeventeenPrincessMononokeExpansion.ts",\n  "modernCanonExpansion.ts",',
  '  "chapterSeventeenPrincessMononokeExpansion.ts",\n  "chapterSeventeenEvesBayouExpansion.ts",\n  "modernCanonExpansion.ts",',
);

const chapterWords = ["twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen"];
for (const word of chapterWords) {
  const script = `scripts/film-history-chapter-${word}-atlas-audit.mjs`;
  replaceOnce(script, "const EXPECTED_ATLAS_COUNT = 487;", "const EXPECTED_ATLAS_COUNT = 488;");
  replaceOnce(
    script,
    '  "chapterSeventeenPrincessMononokeExpansion.ts",\n  "modernCanonExpansion.ts",',
    '  "chapterSeventeenPrincessMononokeExpansion.ts",\n  "chapterSeventeenEvesBayouExpansion.ts",\n  "modernCanonExpansion.ts",',
  );
}

for (const word of ["Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen"]) {
  replaceAllLiteral(`src/core/filmHistoryChapter${word}AuditContract.test.ts`, "487", "488", 3);
}

replaceOnce(
  "src/core/filmHistoryChapterSeventeenAuditContract.test.ts",
  '  "Princess Mononoke",\n  "Festen",',
  '  "Princess Mononoke",\n  "Eve\'s Bayou",\n  "Festen",',
);
replaceOnce(
  "src/core/filmHistoryChapterSeventeenAuditContract.test.ts",
  'const exactP1Queue = [\n  "Eve\'s Bayou",\n  "The Idiots",',
  'const exactP1Queue = [\n  "The Idiots",',
);
replaceOnce(
  "src/core/filmHistoryChapterSeventeenAuditContract.test.ts",
  'const exactRecommended = [\n  "Eve\'s Bayou",\n  "The Idiots",',
  'const exactRecommended = [\n  "The Idiots",',
);

for (const word of chapterWords) {
  execFileSync("node", [`scripts/film-history-chapter-${word}-atlas-audit.mjs`, `--write=docs/film-history-chapter-${word}-atlas-resolved.json`], { stdio: ["ignore", "ignore", "inherit"] });
}

const report = JSON.parse(read("docs/film-history-chapter-seventeen-atlas-resolved.json"));
if (report.atlas.expectedCount !== 488 || report.atlas.actualCount !== 488) throw new Error("Chapter 17 is not 488/488");
if (report.verificationIndex.literalVerifiedScenarioIds !== 488) throw new Error("verification index is not 488");
const item = report.candidates.find((candidate) => candidate.title === "Eve's Bayou");
if (!item || item.decision !== "USE_EXISTING" || item.scenarioId !== "scenario_eves_bayou_1997" || item.productionVerified !== true) {
  throw new Error(`Eve's Bayou did not resolve correctly: ${JSON.stringify(item)}`);
}
if (JSON.stringify(report.byDecision.P0) !== JSON.stringify([])) throw new Error(`Unexpected P0 ${JSON.stringify(report.byDecision.P0)}`);
const expectedP1 = ["The Idiots","Ringu","The Blair Witch Project","Boys Don't Cry"];
if (JSON.stringify(report.byDecision.P1) !== JSON.stringify(expectedP1)) throw new Error(`Unexpected P1 ${JSON.stringify(report.byDecision.P1)}`);
if (JSON.stringify(report.byDecision.P2) !== JSON.stringify(["Goodfellas"])) throw new Error(`Unexpected P2 ${JSON.stringify(report.byDecision.P2)}`);
if (report.recommendedNewProductionCases.includes("Eve's Bayou")) throw new Error("Eve's Bayou still recommended");
execFileSync("node", ["scripts/production-case-rest-audit.mjs"], { stdio: "inherit" });
