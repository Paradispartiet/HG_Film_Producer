...
const SCENARIO_ID = "scenario_bad_luck_banging_or_loony_porn_2021";
...
const TEMP_SCRIPT = "scripts/TEMP-ch19-bad-luck-banging-or-loony-porn-reconcile.mjs";
const TEMP_WORKFLOW = ".github/workflows/TEMP-ch19-bad-luck-banging-or-loony-porn-reconcile.yml";
...
invariant(immutableStudy.includes(`scenarioId: "${SCENARIO_ID}"`), "Existing Bad Luck Film Study identity is missing.");
invariant(immutablePv.includes(`scenarioId: "${SCENARIO_ID}"`), "Existing Bad Luck PV identity is missing.");
...
insertAfterInConstArray(TEST_PATH, "exactCandidateTitles", "There Is No Evil", "Bad Luck Banging or Loony Porn");
insertAfterInConstArray(TEST_PATH, "exactP1Priority", "There Is No Evil", "Bad Luck Banging or Loony Porn");
insertAfterInConstArray(TEST_PATH, "exactUseExisting", "There Is No Evil", "Bad Luck Banging or Loony Porn");
...
replaceOnce(
  ...
  `  assert.equal(thereIsNoEvil.productionVerified, true);\n\n  const badLuck = resolved.candidates.find((candidate) => candidate.title === "Bad Luck Banging or Loony Porn");\n  assert.ok(badLuck);\n  assert.equal(badLuck.decision, "USE_EXISTING");\n  assert.equal(badLuck.scenarioId, "${SCENARIO_ID}");\n  assert.equal(badLuck.matches, 1);\n  assert.equal(badLuck.productionVerified, true);\n\n  for (const candidate of resolved.candidates) {`,
);
...
replaceOnce(
  ...
  `invariant(thereIsNoEvil?.decision === "USE_EXISTING" && thereIsNoEvil?.scenarioId === "scenario_there_is_no_evil_2020" && thereIsNoEvil?.matches === 1 && thereIsNoEvil?.productionVerified === true, "There Is No Evil is not closed as one existing production-verified Chapter 19 case.");\nconst badLuckBanging = chapter19.candidates.find((candidate) => candidate.title === "Bad Luck Banging or Loony Porn");\ninvariant(badLuckBanging?.decision === "USE_EXISTING" && badLuckBanging?.scenarioId === "${SCENARIO_ID}" && badLuckBanging?.matches === 1 && badLuckBanging?.productionVerified === true, "Bad Luck Banging or Loony Porn is not closed as one existing production-verified Chapter 19 case.");\nconst tenet =`,
);
...
const film = resolved.candidates.find((item) => item.title === "Bad Luck Banging or Loony Porn");
invariant(film?.scenarioId === SCENARIO_ID && film?.matches === 1 && film?.productionVerified === true && film?.decision === "USE_EXISTING", "Bad Luck Banging did not resolve to the existing verified identity.");
invariant(resolved.candidates.filter((item) => item.scenarioId === SCENARIO_ID).length === 1, "Bad Luck Banging scenario identity is duplicated in Chapter 19 candidates.");
...
const obligation = festival.obligations.find((item) => item.title === "Bad Luck Banging or Loony Porn");
invariant(obligation?.status === "PRODUCTION_VERIFIED" && obligation?.scenarioId === SCENARIO_ID, `Festival gate did not close Bad Luck Banging: ${obligation?.status}`);
...
invariant(read(FILM_STUDY_PATH) === immutableStudy, "Bad Luck Banging Film Study changed during candidate reconciliation.");
invariant(read(PV_PATH) === immutablePv, "Bad Luck Banging Production Verification changed during candidate reconciliation.");

unlinkSync(TEMP_SCRIPT);
unlinkSync(TEMP_WORKFLOW);
console.log(JSON.stringify({ atlas: `${resolved.atlas.actualCount}/${resolved.atlas.expectedCount}`, productionVerificationIds: resolved.verificationIndex.literalVerifiedScenarioIds, candidates: resolved.candidates.length, useExisting: resolved.byDecision.USE_EXISTING.length, p2: resolved.byDecision.P2.length, badLuckBanging: film, nextCorrection: festival.correctiveQueue[0], immutableStudy: true, immutablePv: true }, null, 2));
