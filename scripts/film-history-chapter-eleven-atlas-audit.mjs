...
function parseDefinitions(text, origin) {
  const scenarios = [];
  const pattern = /id:\s*"(scenario_[^"]+)"[\s\S]{0,1200}?title:\s*"([^"]+)"[\s\S]{0,800}?originalTitle:\s*"([^"]*)"[\s\S]{0,500}?year:\s*(\d{4})/g;
  for (const match of text.matchAll(pattern)) {
    scenarios.push({ id: match[1], title: match[2], originalTitle: match[3], year: Number(match[4]), origin });
  }
  return scenarios;
}
...
function buildAtlas() {
  ...
  for (const scenario of seed.scenarios ?? []) {
    byId.set(scenario.id, {
      ...
      year: Number(scenario.film?.year ?? 0),
      origin: "film_scenarios_seed.json",
    });
  }
  const expansionStats = [];
  for (const fileName of expansionFiles) {
    const filePath = path.join(coreDirectory, fileName);
    const parsed = parseDefinitions(readText(filePath), fileName);
    let appended = 0;
    let matchedExisting = 0;
    for (const scenario of parsed) {
      if (byId.has(scenario.id)) matchedExisting += 1;
      else { byId.set(scenario.id, scenario); appended += 1; }
    }
    expansionStats.push({ fileName, parsed: parsed.length, appended, matchedExisting });
  }
  return { scenarios: [...byId.values()], expansionStats };
}
