...
const SCENARIO_ID = "scenario_annette_2021";
const BASE_ATLAS = 595;
const BASE_PV = 595;
const BASE_CANDIDATES = 72;
const BASE_USE_EXISTING = 70;
const BASE_CANNES_UNRESOLVED = 20;
...
function replaceOnce(p, before, after) {
  ...
  if (first < 0) throw new Error(`${p}: required marker missing: ${before.slice(0, 160)}`);
  if (source.indexOf(before, first + before.length) >= 0) throw new Error(`${p}: marker is not unique: ${before.slice(0, 160)}`);
...
// Prebaseline must come from committed canonical reports because the source-first PV file already contains the next literal ID.
...
if (before.byDecision?.USE_EXISTING?.length !== BASE_USE_EXISTING || before.byDecision?.P2?.length !== 2) throw new Error("Unexpected committed decision census before Annette.");
if (before.candidates?.some((item) => item.title === "Annette" || item.scenarioId === SCENARIO_ID)) throw new Error("Annette is already present in the Chapter 19 candidate matrix; refusing duplicate materialization.");
if (beforeCannes.correctiveQueue?.length !== BASE_CANNES_UNRESOLVED || beforeCannes.correctiveQueue?.[0]?.title !== "Annette") throw new Error(`Expected Annette as leader of a ${BASE_CANNES_UNRESOLVED}-film Cannes queue.`);
if (beforeCannes.correctiveQueue?.[0]?.status !== "MISSING_CANDIDATE") throw new Error(`Annette must enter materialization from MISSING_CANDIDATE, found ${beforeCannes.correctiveQueue?.[0]?.status}.`);

const allowedIdentityPaths = new Set([
  "src/core/chapterNineteenAnnetteExpansion.ts",
  "src/core/chapterNineteenAnnetteExpansion.test.ts",
  "src/ui/data/scenarioFilmStudyChapterNineteenAnnette.ts",
  "src/ui/data/scenarioProductionVerificationAnnette.ts",
  "scripts/TEMP-materialize-ch19-annette.mjs",
  ".github/workflows/TEMP-ch19-annette-materialize.yml",
]);
const identityHits = [];
...
if (identityHits.length) throw new Error(`Pre-existing Annette scenario identity found outside source-first files: ${identityHits.join(", ")}`);
...
replaceOnce(files.filmScenarios,
  '+manual_chapter_nineteen_compartment_no_6_expansion_2026+manual_chapter_nineteen_eo_expansion_2026',
  '+manual_chapter_nineteen_compartment_no_6_expansion_2026+manual_chapter_nineteen_annette_expansion_2026+manual_chapter_nineteen_eo_expansion_2026');
...
replaceOnce(files.chapter19Audit,
  ...
  '${triangleCandidate}${drommerCandidate}${happeningCandidate}${allBeautyCandidate}${roomNextDoorCandidate}${thereIsNoEvilCandidate}${badLuckCandidate}${alcarrasCandidate}${adamantCandidate}${fatherMotherCandidate}${aHeroCandidate}${compartmentNo6Candidate}${annetteCandidate}`);');

replaceOnce(files.chapter18Completion,
  `invariant(verificationIds.size === ${BASE_PV}, \`Global Production Verification registry must contain exactly ${BASE_PV} unique scenarioIds after Compartment No. 6 Chapter 19 materialization: \${verificationIds.size}\`);`,
  `invariant(verificationIds.size === ${NEXT_PV}, \`Global Production Verification registry must contain exactly ${NEXT_PV} unique scenarioIds after Annette Chapter 19 materialization: \${verificationIds.size}\`);`);
replaceOnce(files.chapter18Completion,
...
replaceOnce(files.chapter19Contract,
  '  "Compartment No. 6",\n  "Nomadland",',
  '  "Compartment No. 6",\n  "Annette",\n  "Nomadland",');
replaceOnce(files.chapter19Contract, 'assert.match(audit, /const EXPECTED_ATLAS_COUNT = 595;/);', 'assert.match(audit, /const EXPECTED_ATLAS_COUNT = 596;/);');
replaceOnce(files.chapter19Contract, 'assert.equal(resolved.atlas.expectedCount, 595);', 'assert.equal(resolved.atlas.expectedCount, 596);');
replaceOnce(files.chapter19Contract, 'assert.equal(resolved.atlas.actualCount, 595);', 'assert.equal(resolved.atlas.actualCount, 596);');
replaceOnce(files.chapter19Contract, 'assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 595);', 'assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 596);');
replaceOnce(files.chapter19Contract, 'test("Chapter 19 locks exactly seventy-two candidates across 2020-2025", () => {', 'test("Chapter 19 locks exactly seventy-three candidates across 2020-2025", () => {');
replaceOnce(files.chapter19Contract, 'assert.equal(new Set(resolved.candidates.map((item) => item.title)).size, 72);', 'assert.equal(new Set(resolved.candidates.map((item) => item.title)).size, 73);');
replaceOnce(files.chapter19Contract, 'assert.equal(resolved.candidates.length, 72);', 'assert.equal(resolved.candidates.length, 73);');
replaceOnce(files.chapter19Contract, 'const expectedCandidatesByYear = new Map([[2020, 11], [2021, 14], [2022, 13], [2023, 11], [2024, 12], [2025, 11]]);', 'const expectedCandidatesByYear = new Map([[2020, 11], [2021, 15], [2022, 13], [2023, 11], [2024, 12], [2025, 11]]);');
replaceOnce(files.chapter19Contract, 'assert.equal(exactP1Priority.length, 45);', 'assert.equal(exactP1Priority.length, 46);');
replaceOnce(files.chapter19Contract, 'assert.equal(exactUseExisting.length, 70);', 'assert.equal(exactUseExisting.length, 71);');
replaceOnce(files.chapter19Contract,
  '  assert.equal(compartmentNo6.productionVerified, true);\n\n  const tenet = resolved.candidates.find((candidate) => candidate.title === "Tenet");',
  '  assert.equal(compartmentNo6.productionVerified, true);\n\n  const annette = resolved.candidates.find((candidate) => candidate.title === "Annette");\n  assert.ok(annette);\n  assert.equal(annette.year, 2021);\n  assert.equal(annette.decision, "USE_EXISTING");\n  assert.equal(annette.scenarioId, "scenario_annette_2021");\n  assert.equal(annette.matches, 1);\n  assert.equal(annette.productionVerified, true);\n\n  const tenet = resolved.candidates.find((candidate) => candidate.title === "Tenet");');
...
if (after.atlas?.actualCount !== NEXT_ATLAS || after.atlas?.expectedCount !== NEXT_ATLAS) throw new Error(`Annette materialization did not produce exact ${NEXT_ATLAS}/${NEXT_ATLAS}.`);
if (after.verificationIndex?.literalVerifiedScenarioIds !== NEXT_PV) throw new Error(`Annette materialization did not produce exact ${NEXT_PV} PV IDs.`);
if (after.candidates?.length !== NEXT_CANDIDATES) throw new Error(`Annette materialization did not produce exact ${NEXT_CANDIDATES} candidates.`);
if (after.byDecision?.USE_EXISTING?.length !== NEXT_USE_EXISTING || after.byDecision?.P2?.length !== 2) throw new Error(`Decision census drifted after Annette: ${JSON.stringify(after.byDecision)}`);
const annette = after.candidates.find((item) => item.title === "Annette");
if (!annette || annette.scenarioId !== SCENARIO_ID || annette.matches !== 1 || annette.productionVerified !== true || annette.decision !== "USE_EXISTING") throw new Error(`Annette did not close as one production-verified identity: ${JSON.stringify(annette)}`);
const expansion = after.atlas?.expansionOrder?.find((item) => item.fileName === "chapterNineteenAnnetteExpansion.ts");
if (!expansion || expansion.definitions !== 1 || expansion.appended !== 1 || expansion.matchedExisting !== 0) throw new Error(`Annette must be proven as appended:1/matchedExisting:0, found ${JSON.stringify(expansion)}.`);
...
if (afterCannes.summary?.unresolvedFilms !== BASE_CANNES_UNRESOLVED - 1 || afterCannes.summary?.missingCandidateFilms !== BASE_CANNES_UNRESOLVED - 1) throw new Error(`Cannes queue did not reduce exactly 20→19: ${JSON.stringify(afterCannes.summary)}`);
if (afterCannes.correctiveQueue?.length !== BASE_CANNES_UNRESOLVED - 1 || afterCannes.correctiveQueue?.[0]?.title !== "Ahed's Knee") throw new Error(`Expected Ahed's Knee as next Cannes leader after Annette, found ${afterCannes.correctiveQueue?.[0]?.title}.`);
const awardFilm = afterCannes.awardedFilms?.find((item) => item.title === "Annette");
if (!awardFilm || awardFilm.status !== "PRODUCTION_VERIFIED" || awardFilm.scenarioId !== SCENARIO_ID || awardFilm.atlasMatches !== 1) throw new Error(`Cannes audit did not close Annette: ${JSON.stringify(awardFilm)}`);
