...
test("Chapter 19 preserves the closed Chapter 18 baseline while advancing the current Atlas", () => {
  ...
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 549;/);
  assert.equal(resolved.atlas.baselineFromClosedChapter18, 539);
  assert.equal(resolved.atlas.expectedCount, 549);
  assert.equal(resolved.atlas.actualCount, 549);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 549);
});
...
test("Chapter 19 locks the source-first priority model and resolved queues", () => {
  ...
  assert.equal(exactUseExisting.length, 17);
  assert.equal(exactP0Queue.length, 15);
  assert.equal(exactP1Queue.length, 24);
  ...
  assert.equal(resolved.productionStrategy.nextRecommendedCase, "Top Gun: Maverick");
