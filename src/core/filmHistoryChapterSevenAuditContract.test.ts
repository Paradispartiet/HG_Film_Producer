...
test("Chapter 7 audit locks the reviewed Weimar production-case matrix", () => {
  ...
  assert.match(packageJson, /npm run audit:film-history-ch6 && npm run audit:film-history-ch7 && npm run audit:film-history-ch8 && npm run audit:film-history-ch9(?: && npm run audit:film-history-ch\d+)* && npm run typecheck/);
});
