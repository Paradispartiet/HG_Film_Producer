...
test("Chapter 8 audit locks the reviewed French avant-garde Production Case matrix", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 406;/);
  ...
  assert.match(audit, /USE_EXISTING: \["Cœur fidèle", "The Passion of Joan of Arc"\]/);
  assert.match(audit, /P0: \["Napoléon", "Un Chien Andalou"\]/);
  ...
  assert.deepEqual(resolved.byDecision.USE_EXISTING, ["The Passion of Joan of Arc", "Cœur fidèle"]);
  assert.deepEqual(resolved.byDecision.P0, ["Napoléon", "Un Chien Andalou"]);
  assert.deepEqual(resolved.byDecision.P1, ["The Smiling Madame Beudet", "Entr'acte"]);
  assert.deepEqual(resolved.recommendedNewProductionCases, ["Napoléon", "Un Chien Andalou", "The Smiling Madame Beudet", "Entr'acte"]);
