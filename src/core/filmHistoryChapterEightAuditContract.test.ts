...
test("Chapter 8 audit locks the reviewed French avant-garde Production Case matrix", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 407;/);
  ...
  assert.match(audit, /USE_EXISTING: \["Cœur fidèle", "Napoléon", "The Passion of Joan of Arc"\]/);
  assert.match(audit, /P0: \["Un Chien Andalou"\]/);
  ...
  assert.deepEqual(resolved.byDecision.USE_EXISTING, ["The Passion of Joan of Arc", "Cœur fidèle", "Napoléon"]);
  assert.deepEqual(resolved.byDecision.P0, ["Un Chien Andalou"]);
  assert.deepEqual(resolved.byDecision.P1, ["The Smiling Madame Beudet", "Entr'acte"]);
  assert.deepEqual(resolved.recommendedNewProductionCases, ["Un Chien Andalou", "The Smiling Madame Beudet", "Entr'acte"]);
