...
const exactExisting = [
  ...
  "Poetry", "Somewhere", "Uncle Boonmee Who Can Recall His Past Lives", "A Separation", "Pietà", "Gravity", "Blue Is the Warmest Colour", "Birdman", "Boyhood", "Winter Sleep", "Black Coal, Thin Ice",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 527;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 527);
  assert.equal(resolved.atlas.actualCount, 527);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 527);
});
