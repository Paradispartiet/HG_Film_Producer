...
const exactExisting = [
  "The Gleaners and I", "Platform", "Dancer in the Dark", "In the Mood for Love", "Yi Yi", "Crouching Tiger, Hidden Dragon", "Atanarjuat: The Fast Runner", "Spirited Away", "The Son's Room", "Millennium Mambo", "Russian Ark", "City of God", "Star Wars: Episode II - Attack of the Clones", "28 Days Later", "Unknown Pleasures", "Lost in Translation", "The Return", "Collateral", "Tropical Malady",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 519;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 519);
  assert.equal(resolved.atlas.actualCount, 519);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 519);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 62);
