...
const exactExisting = [
  "The Gleaners and I", "Platform", "Dancer in the Dark", "In the Mood for Love", "Atanarjuat: The Fast Runner", "The Son's Room", "Millennium Mambo", "Lost in Translation", "The Return", "Tropical Malady",
...
const exactP0Queue = [
  "Spirited Away", "Russian Ark", "City of God",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 496;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 496);
  assert.equal(resolved.atlas.actualCount, 496);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 496);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 39);
  assert.equal(exactP0Queue.length, 19);
