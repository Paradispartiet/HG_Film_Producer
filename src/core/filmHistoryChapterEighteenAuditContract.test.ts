...
const exactExisting = [
  ...
  "Poetry", "Somewhere", "A Separation", "Pietà", "Blue Is the Warmest Colour", "Birdman", "Boyhood", "Winter Sleep", "Black Coal, Thin Ice",
  "Tangerine", "Mad Max: Fury Road", "From Afar", "Toni Erdmann", "Burning", "An Elephant Sitting Still", "Synonyms",
  "Portrait of a Lady on Fire",
] as const;

const exactP0Queue = [
  "Gravity", "Son of Saul", "Moonlight", "Dunkirk", "Roma",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 507;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 507);
  assert.equal(resolved.atlas.actualCount, 507);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 507);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 50);
