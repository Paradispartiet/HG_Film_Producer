...
const exactP0Queue = [
  "Son of Saul", "Moonlight", "Dunkirk", "Roma",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 508;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 508);
  assert.equal(resolved.atlas.actualCount, 508);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 508);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 51);
