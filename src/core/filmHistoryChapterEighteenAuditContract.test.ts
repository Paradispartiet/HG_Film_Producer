...
const exactExisting = [
  ...
  "Head-On", "Vera Drake", "Brokeback Mountain", "Caché", "Inland Empire", "Still Life", "Children of Men", "Pan's Labyrinth", "Apocalypto", "Zodiac", "Secret Sunshine", "No Country for Old Men", "4 Months, 3 Weeks and 2 Days", "Slumdog Millionaire",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 525;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 525);
  assert.equal(resolved.atlas.actualCount, 525);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 525);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 68);
