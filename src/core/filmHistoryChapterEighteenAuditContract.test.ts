...
const exactExisting = [
  "The Gleaners and I", "Platform", "Dancer in the Dark", "In the Mood for Love", "Yi Yi", "Crouching Tiger, Hidden Dragon", "Atanarjuat: The Fast Runner", "Spirited Away", "The Son's Room", "Millennium Mambo", "Russian Ark", "City of God", "Star Wars: Episode II - Attack of the Clones", "Lost in Translation", "The Return", "Collateral", "Tropical Malady",
...
const exactP1Queue = [
  "28 Days Later", "Unknown Pleasures", "Oldboy", "Brokeback Mountain", "Caché",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 517;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 517);
  assert.equal(resolved.atlas.actualCount, 517);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 517);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 60);
  assert.equal(exactP0Queue.length, 0);
  assert.equal(exactP1Queue.length, 21);
  assert.equal(exactP2Queue.length, 1);
  assert.equal(resolved.recommendedNewProductionCases.length, 21);
  assert.deepEqual(
