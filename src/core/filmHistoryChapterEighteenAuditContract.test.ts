...
const exactExisting = [
  ...
  "Head-On", "Vera Drake", "Brokeback Mountain", "Caché", "The New World", "Inland Empire", "Still Life", "Children of Men", "Pan's Labyrinth", "Apocalypto", "Zodiac", "Secret Sunshine", "No Country for Old Men", "There Will Be Blood", "4 Months, 3 Weeks and 2 Days", "Slumdog Millionaire",
  ...
  "Poetry", "Somewhere", "Uncle Boonmee Who Can Recall His Past Lives", "A Separation", "The Tree of Life", "Pina", "Amour", "Pietà", "Holy Motors", "Gravity", "Ida", "Blue Is the Warmest Colour", "Birdman", "Boyhood", "Winter Sleep", "Black Coal, Thin Ice",
  "Tangerine", "Mad Max: Fury Road", "Son of Saul", "The Revenant", "From Afar", "Moonlight", "Toni Erdmann", "I Am Not Your Negro", "Dunkirk", "Get Out", "Roma", "Burning", "Spider-Man: Into the Spider-Verse", "An Elephant Sitting Still", "Cold War", "Parasite", "The Irishman", "1917", "Atlantics", "Synonyms",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 539;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 539);
  assert.equal(resolved.atlas.actualCount, 539);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 539);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 82);
  assert.equal(exactP0Queue.length, 0);
  assert.equal(exactP1Queue.length, 0);
  assert.equal(exactP2Queue.length, 0);
