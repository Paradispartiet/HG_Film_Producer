...
const exactExisting = [
  "The Gleaners and I", "Platform", "Dancer in the Dark", "In the Mood for Love", "The Son's Room", "Millennium Mambo", "Lost in Translation", "The Return", "Tropical Malady",
...
const exactP0Queue = [
  "Atanarjuat: The Fast Runner", "Spirited Away", "Russian Ark", "City of God",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 495;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 495);
  assert.equal(resolved.atlas.actualCount, 495);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 495);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 38);
  assert.equal(exactP0Queue.length, 20);
