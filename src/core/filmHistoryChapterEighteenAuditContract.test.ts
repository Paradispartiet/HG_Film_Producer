...
const exactExisting = [
  ...
  "Tangerine", "Mad Max: Fury Road", "Son of Saul", "From Afar", "Moonlight", "Toni Erdmann", "Dunkirk", "Roma", "Burning", "An Elephant Sitting Still", "Synonyms",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 512;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 512);
  assert.equal(resolved.atlas.actualCount, 512);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 512);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 55);
