...
const exactExisting = [
  ...
  "Tangerine", "Mad Max: Fury Road", "Son of Saul", "The Revenant", "From Afar", "Moonlight", "Toni Erdmann", "I Am Not Your Negro", "Dunkirk", "Roma", "Burning", "Spider-Man: Into the Spider-Verse", "An Elephant Sitting Still", "Parasite", "The Irishman", "Synonyms",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 534;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 534);
  assert.equal(resolved.atlas.actualCount, 534);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 534);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 77);
  assert.equal(exactP0Queue.length, 0);
  assert.equal(exactP1Queue.length, 4);
