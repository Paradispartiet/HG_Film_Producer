...
const exactExisting = [
  "The Gleaners and I", "Platform", "Dancer in the Dark", "In the Mood for Love", "Atanarjuat: The Fast Runner", "Spirited Away", "The Son's Room", "Millennium Mambo", "Lost in Translation", "The Return", "Tropical Malady",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 497;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 497);
  assert.equal(resolved.atlas.actualCount, 497);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 497);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 40);
  assert.equal(exactP0Queue.length, 18);
  assert.equal(exactP1Queue.length, 23);
  assert.equal(exactP2Queue.length, 1);
  assert.equal(resolved.recommendedNewProductionCases.length, 41);
  assert.deepEqual(
