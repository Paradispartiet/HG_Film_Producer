...
const exactExisting = [
  "The Gleaners and I", "Platform", "Dancer in the Dark", "In the Mood for Love", "Atanarjuat: The Fast Runner", "Spirited Away", "The Son's Room", "Millennium Mambo", "Russian Ark", "City of God", "Lost in Translation", "The Return", "Tropical Malady",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 499;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 499);
  assert.equal(resolved.atlas.actualCount, 499);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 499);
});
