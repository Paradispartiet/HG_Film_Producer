...
const exactExisting = [
  "The Gleaners and I", "Platform", "Dancer in the Dark", "In the Mood for Love", "Atanarjuat: The Fast Runner", "Spirited Away", "The Son's Room", "Millennium Mambo", "Russian Ark", "City of God", "Star Wars: Episode II - Attack of the Clones", "Lost in Translation", "The Return", "Tropical Malady",
  "Head-On", "Vera Drake", "Still Life", "Secret Sunshine", "No Country for Old Men", "4 Months, 3 Weeks and 2 Days",
...
const exactP0Queue = [
  "Collateral", "Inland Empire", "Zodiac", "Slumdog Millionaire", "Avatar",
  "The Social Network", "A Separation", "Gravity", "Son of Saul", "Moonlight", "Dunkirk", "Roma",
...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 500;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 500);
  assert.equal(resolved.atlas.actualCount, 500);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 500);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 43);
  assert.equal(exactP0Queue.length, 15);
  assert.equal(exactP1Queue.length, 23);
  assert.equal(exactP2Queue.length, 1);
  assert.equal(resolved.recommendedNewProductionCases.length, 38);
  assert.deepEqual(
