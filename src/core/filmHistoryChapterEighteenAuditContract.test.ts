...
test("Chapter 18 audit locks the 2000–2019 digital-convergence scope", () => {
  assert.match(audit, /const EXPECTED_ATLAS_COUNT = 526;/);
  ...
  assert.equal(resolved.atlas.expectedCount, 526);
  assert.equal(resolved.atlas.actualCount, 526);
  assert.equal(resolved.verificationIndex.literalVerifiedScenarioIds, 526);
});
...
test("Chapter 18 locks the exact evidence-derived Production Case queues", () => {
  ...
  assert.equal(exactExisting.length, 69);
