...
import { getEditingSoundLessonCopy } from "../../core/filmSchoolEditingSoundLessonCopy";
import {
...
export function FilmSchoolEditingSoundCourse({
  ...
  const activeCopy = getEditingSoundLessonCopy(language, activeLesson);
      ...
      <main className="film-school-course-page">
        ...
        <section className="school-course-workspace">
          <aside className="school-lesson-rail">
            ...
            <div>{EDITING_SOUND_COURSE_LESSONS.map((lesson) => { const lessonStage = getEditingSoundLessonMasteryStage(progress, lesson.id); const lessonCopy = getEditingSoundLessonCopy(language, lesson); return <button className={lesson.id === activeLesson.id ? "school-lesson-button is-active" : "school-lesson-button"} key={lesson.id} onClick={() => selectLesson(lesson.id)} type="button"><span>{lesson.number}</span><div><strong>{lessonCopy.title}</strong><small className={`stage-${lessonStage}`}>{stageLabels[lessonStage]}</small></div></button>; })}</div>
          </aside>
