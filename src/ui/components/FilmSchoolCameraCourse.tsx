...
type FilmSchoolCameraCourseProps = {
...
export function FilmSchoolCameraCourse({
  navigate,
  onOpenAtlas,
  onOpenDirector,
  onSelectCourse,
  scenarios,
}: FilmSchoolCameraCourseProps) {
  const [language] = useFilmWorkLanguage();
  const chrome = FILM_SCHOOL_COURSE_CHROME[language];
  const stageLabels = chrome.stage;
  const [progress, setProgress] = useState<CameraCourseProgress>(() => loadProgress());
  const [quizAnswers, setQuizAnswers] = useState<Readonly<Record<string, number>>>({});
  const lessonExamples = useMemo(
    () => CAMERA_COURSE_LESSONS.map((lesson, index) => resolveLessonScenario(scenarios, lesson, index)).filter((scenario): scenario is FilmScenarioSeed => Boolean(scenario)),
    [scenarios],
  );
  const [assignmentFilmId, setAssignmentFilmId] = useState(() => lessonExamples[0]?.id ?? scenarios[0]?.id ?? "");
  const activeLesson = CAMERA_COURSE_LESSONS.find((lesson) => lesson.id === progress.activeLessonId) ?? CAMERA_COURSE_LESSONS[0];
  const activeIndex = activeLesson ? CAMERA_COURSE_LESSONS.findIndex((lesson) => lesson.id === activeLesson.id) : 0;
  ...
  const completionPercent = getCameraCourseCompletionPercent(progress);
  const mastered = isCameraCourseMastered(progress);
  const masteredCount = CAMERA_COURSE_LESSONS.filter((lesson) => getCameraLessonMasteryStage(progress, lesson.id) === "mastered").length;
  const exampleInsights = activeBrief ? activeBrief.cinematographyTargets.slice(0, 4) : [];
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(CAMERA_COURSE_PROGRESS_STORAGE_KEY, JSON.stringify(progress));
    } catch {
  ...
  function selectLesson(lessonId: string) {
    ...
    window.setTimeout(() => document.getElementById("school-active-camera-lesson")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  }

  function answerQuiz(lesson: CameraCourseLesson, answerIndex: number) {
    setQuizAnswers((current) => ({ ...current, [lesson.id]: answerIndex }));
    if (!isCameraCourseQuizAnswerCorrect(lesson, answerIndex)) return;
  ...
  function resetCourse() {
    ...
    setProgress(createBlankCameraCourseProgress());
    setQuizAnswers({});
  }
  ...
  function startDirectorAssignment() {
    ...
    try {
      window.localStorage.setItem(CAMERA_DIRECTOR_ASSIGNMENT_STORAGE_KEY, JSON.stringify(assignment));
    } catch {
  ...
  const quizCorrect = selectedAnswer !== undefined && isCameraCourseQuizAnswerCorrect(activeLesson, selectedAnswer);
  const practiceNote = progress.notesByLessonId[activeLesson.id] ?? "";
  const stage = getCameraLessonMasteryStage(progress, activeLesson.id);
  const terms = getCameraCourseLessonTerms(activeLesson);
  const remainingModules = CAMERA_COURSE_LESSONS.length - masteredCount;

  return (
    <div className="filmverket-shell school-course-shell school-camera-course-shell">
      ...
      <main className="film-school-course-page">
        <FilmSchoolCourseNavigation activeCourseId="camera" language={language} onSelectCourse={onSelectCourse} />

        <section className="school-course-hero school-course-hero--camera">
          <div>
            <span className="filmverket-kicker">Film School · {chrome.foundation} · 3/5</span>
            <h1>Bilde, kamera og <em>optikk</em></h1>
            <p>Gjør hvert utsnitt, ståsted, objektiv, fokusskift og kameratrekk til en konkret dramatisk beslutning.</p>
          </div>
          <aside>
            ...
            <p>{masteredCount} / {CAMERA_COURSE_LESSONS.length} {chrome.modules}</p>
          </aside>
        </section>

        <section className="school-course-summary">
          <div><strong>{CAMERA_COURSE_LESSONS.length}</strong><span>{chrome.modules}</span></div>
          <div><strong>{new Set(CAMERA_COURSE_LESSONS.flatMap((lesson) => lesson.termIds)).size}</strong><span>{chrome.terms}</span></div>
          <div><strong>{CAMERA_COURSE_LESSONS.length}</strong><span>{chrome.filmExamples}</span></div>
        ...
        <section className="school-course-workspace">
          ...
          <article className="school-active-lesson" id="school-active-camera-lesson">
            ...
            <section className="school-practice-card">
              ...
              <textarea onChange={(event) => updatePracticeNote(activeLesson.id, event.target.value)} placeholder="Skriv bilde-, kamera- eller optikkplanen din her …" rows={7} value={practiceNote} />
            ...
            <nav className="school-lesson-navigation" aria-label={chrome.courseModuleNavAria}>
              <button disabled={activeIndex <= 0} onClick={() => { const previous = CAMERA_COURSE_LESSONS[activeIndex - 1]; if (previous) selectLesson(previous.id); }} type="button">{chrome.previousModule}</button>
              <button disabled={activeIndex >= CAMERA_COURSE_LESSONS.length - 1} onClick={() => { const next = CAMERA_COURSE_LESSONS[activeIndex + 1]; if (next) selectLesson(next.id); }} type="button">{chrome.nextModule}</button>
            </nav>
          </article>
        </section>

        <section className={mastered ? "school-final-assignment is-unlocked" : "school-final-assignment"}>
          <div><span className="filmverket-kicker">{chrome.finalAssignment}</span><h2>Fra sceneintensjon til kamerasystem</h2><p>Planlegg én scene der utsnitt, kameraposisjon, objektiv, bevegelse og fokus følger scenens dramatiske utvikling. Oppgaven åpnes i Film Director med fem leveransefelt.</p></div>
          <aside>
      ...
      <footer className="filmverket-footer"><span>{chrome.productBrand} · Film School</span><span>Utsnitt → perspektiv → optikk → bevegelse → fokus</span></footer>
    </div>
  );
}

function resolveLessonScenario(scenarios: readonly FilmScenarioSeed[], lesson: CameraCourseLesson, fallbackIndex: number): FilmScenarioSeed | undefined {
...
function loadProgress(): CameraCourseProgress {
  try {
    return coerceCameraCourseProgress(JSON.parse(window.localStorage.getItem(CAMERA_COURSE_PROGRESS_STORAGE_KEY) ?? "null") as unknown);
  } catch {
    return createBlankCameraCourseProgress();
  }
}
