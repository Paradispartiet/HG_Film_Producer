...
const stageLabels: Record<CameraCourseMasteryStage, string> = {
...
export function FilmSchoolCameraCourse({
  ...
  useEffect(() => {
    document.title = "Bilde, kamera og optikk · Film School · Filmverket";
  }, []);
  ...
  return (
    <div className="filmverket-shell school-course-shell school-camera-course-shell">
      ...
      <main className="film-school-course-page">
        <FilmSchoolCourseNavigation activeCourseId="camera" onSelectCourse={onSelectCourse} />

        <section className="school-course-hero school-course-hero--camera">
          <div>
            <span className="filmverket-kicker">Film School · Regi grunnkurs · Kapittel 3</span>
          ...
          <aside>
            ...
            <p>{masteredCount} av {CAMERA_COURSE_LESSONS.length} leksjoner mestret</p>
          </aside>
        </section>

        <section className="school-course-summary">
          <div><strong>{CAMERA_COURSE_LESSONS.length}</strong><span>leksjoner</span></div>
          <div><strong>{new Set(CAMERA_COURSE_LESSONS.flatMap((lesson) => lesson.termIds)).size}</strong><span>fagbegreper</span></div>
          <div><strong>{CAMERA_COURSE_LESSONS.length}</strong><span>filmeksempler</span></div>
        ...
        <section className="school-course-workspace">
          ...
          <article className="school-active-lesson" id="school-active-camera-lesson">
            ...
            <nav className="school-lesson-navigation" aria-label="Course lesson navigation">
              <button disabled={activeIndex <= 0} onClick={() => { const previous = CAMERA_COURSE_LESSONS[activeIndex - 1]; if (previous) selectLesson(previous.id); }} type="button">← Forrige leksjon</button>
              <button disabled={activeIndex >= CAMERA_COURSE_LESSONS.length - 1} onClick={() => { const next = CAMERA_COURSE_LESSONS[activeIndex + 1]; if (next) selectLesson(next.id); }} type="button">Neste leksjon →</button>
            </nav>
          </article>
        </section>
      ...
      <footer className="filmverket-footer"><span>Filmverket · Film School</span><span>Utsnitt → perspektiv → optikk → bevegelse → fokus</span></footer>
    </div>
  );
}
