...
export function ColorDecisionPanel({ options, selectedId, onSelect }: { readonly options: readonly ColorDecision[]; readonly selectedId: string; readonly onSelect: (id: string) => void }) {
  return <section className="post-decision-section"><div className="post-section-heading"><div><span className="eyebrow">Color</span><h3>Finish the image</h3></div><p>Lock the visual treatment and emotional atmosphere of the final cut.</p></div><div className="post-option-grid">{options.map((option) => <DecisionCard effects={[`Visual ${signed(option.visualStyleEffect)}`, `Mood ${signed(option.moodEffect)}`]} key={option.id} onSelect={onSelect} option={option} selected={option.id === selectedId} />)}</div></section>;
}
