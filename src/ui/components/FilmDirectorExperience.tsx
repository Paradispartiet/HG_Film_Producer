  ...
  return (
    <div className="filmverket-shell film-director-shell">
      ...
      <footer className="filmverket-footer">
        ...
        <span>Project · scenes · directing briefs · shot cards</span>
      </footer>
    </div>
  );
}
...
function DirectorProjectEditor({ navigate, scenarios, selectedScenario }: {
  ...
  useEffect(() => {
    try {
      ...
      // Film Director remains usable if persistent browser storage is unavailable.
    }
  ...
  function addScene() {
    setProject((current) => addDirectorScene(current, createDirectorEntityId("scene")));
  }
  ...
  function duplicateActiveScene() {
    setProject((current) => {
      ...
      return duplicateDirectorScene(
        current,
        source.id,
        createDirectorEntityId("scene"),
        source.shots.map(() => createDirectorEntityId("shot")),
      );
    });
  }
  ...
  function deleteActiveScene() {
    ...
    const title = activeScene.brief.sceneTitle.trim() || "this scene";
    if (!window.confirm(`Delete ${title} and all of its shot cards?`)) return;
    setProject((current) => removeDirectorScene(current, activeScene.id, current.scenes[0]?.id ?? ""));
  }
  ...
  function useReferencePrinciple(principle: string) {
    const current = activeScene.brief.formalStrategy;
    if (current.includes(principle)) return;
    const separator = current.trim() ? "\n• " : "• ";
    updateBriefField("formalStrategy", `${current.trimEnd()}${separator}${principle}`);
  }

  function useToneStartingPoint() {
    updateBriefField("audienceEffect", referenceBrief.toneTargets.join("\n• ").replace(/^/, "• "));
  }

  function useCraftStartingPoint() {
    updateBriefField("formalStrategy", activePrinciples.join("\n• ").replace(/^/, "• "));
  }
  ...
  function changeShot(shotId: string, field: DirectorShotFieldId, value: string) {
    setProject((current) => updateDirectorShot(current, current.activeSceneId, shotId, field, value));
  }

  function duplicateShot(shotId: string) {
    setProject((current) => duplicateDirectorShot(current, current.activeSceneId, shotId, createDirectorEntityId("shot")));
  }

  function deleteShot(shotId: string) {
    setProject((current) => removeDirectorShot(current, current.activeSceneId, shotId));
  }

  function moveShot(shotId: string, direction: -1 | 1) {
    setProject((current) => moveDirectorShot(current, current.activeSceneId, shotId, direction));
  }
  ...
  async function copyText(text: string, successState: "scene" | "project") {
    try {
  ...
  return (
    <main className="film-director-page director-project-page">
      <section className="film-director-hero director-project-hero">
        <div>
          <span className="filmverket-kicker">Multi-scene directing workspace</span>
          <h1>Film <em>Director</em></h1>
          <p>Build one directing project from connected scenes. Give each scene its own dramatic plan, then translate that plan into an ordered shot list.</p>
        </div>
        <aside className="film-director-progress-card director-project-progress-card">
          ...
          <p>{project.scenes.length} scene{project.scenes.length === 1 ? "" : "s"} · {totalShots} shot{totalShots === 1 ? "" : "s"}</p>
        </aside>
      </section>
      ...
      <section className="film-director-toolbar director-project-toolbar">
        ...
        <div className="film-director-toolbar-actions">
          ...
          <button className="filmverket-primary-action" onClick={() => copyText(buildDirectorProjectText(project), "project")} type="button">
            {copyState === "project" ? "Project copied" : copyState === "failed" ? "Copy failed" : "Copy complete project"}
          </button>
        </div>
      </section>
      ...
      <section className="director-project-workspace">
        <aside className="director-scene-sidebar">
          <header>
            <div><span>Project scenes</span><strong>{project.scenes.length}</strong></div>
            <button onClick={addScene} type="button">+ Add scene</button>
          </header>
          <div className="director-scene-list">
            {project.scenes.map((scene, index) => {
              const complete = countCompletedDirectorBriefFields(scene.brief);
              return (
                <button
                  className={scene.id === project.activeSceneId ? "director-scene-card director-scene-card--active" : "director-scene-card"}
                  key={scene.id}
                  onClick={() => selectScene(scene.id)}
                  type="button"
                >
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <strong>{scene.brief.sceneTitle.trim() || `Scene ${index + 1}`}</strong>
                  <small>{complete}/{DIRECTOR_BRIEF_FIELDS.length} decisions · {scene.shots.length} shots</small>
                </button>
              );
            })}
          </div>
          <div className="director-scene-sidebar-actions">
            <button onClick={duplicateActiveScene} type="button">Duplicate scene</button>
            <button disabled={project.scenes.length <= 1} onClick={deleteActiveScene} type="button">Delete scene</button>
          </div>
        ...
        <div className="director-active-scene" id="director-active-scene">
          <ActiveSceneHeading
            copyState={copyState}
            onCopy={() => copyText(buildActiveSceneText(project, activeScene), "scene")}
            project={project}
            scene={activeScene}
          />
            ...
            <div className="film-director-principles">
              {activePrinciples.map((principle, index) => (
                <article key={principle}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <p>{principle}</p>
                  <button onClick={() => useReferencePrinciple(principle)} type="button">Use in scene</button>
                </article>
              ))}
            </div>
          ...
          <section className="film-director-form" aria-label="Active scene directing brief">
              ...
              <section className="film-director-form-group" key={group}>
                ...
                <div className="film-director-fields">
                  {fieldLayout.filter((field) => field.group === group).map((field) => {
                    ...
                    const complete = activeScene.brief[field.id].trim().length > 0;
                    return (
                      <label className={complete ? "film-director-field film-director-field--complete" : "film-director-field"} key={field.id}>
                        ...
                        <p>{field.prompt}</p>
                        {field.rows === 1 ? (
                          <input onChange={(event) => updateBriefField(field.id, event.target.value)} type="text" value={activeScene.brief[field.id]} />
                        ) : (
                          <textarea onChange={(event) => updateBriefField(field.id, event.target.value)} rows={field.rows} value={activeScene.brief[field.id]} />
                        )}
                      </label>
                    );
                  })}
      ...
      <section className="film-director-summary director-project-summary">
        ...
        <div><strong>{project.scenes.length}</strong><span>scenes in project</span></div>
        <div><strong>{totalShots}</strong><span>shot cards planned</span></div>
        <div><strong>{completedFieldCount}</strong><span>decisions defined</span></div>
      </section>
    </main>
  );
}
...
function ActiveSceneHeading({ copyState, onCopy, project, scene }: {
  ...
  const completed = countCompletedDirectorBriefFields(scene.brief);
  return (
    <section className="director-active-scene-heading">
      <div>
        <span>Scene {Math.max(1, index + 1)} of {project.scenes.length}</span>
        <h2>{scene.brief.sceneTitle.trim() || `Scene ${index + 1}`}</h2>
        <p>{completed}/{DIRECTOR_BRIEF_FIELDS.length} directing decisions · {scene.shots.length} shot{scene.shots.length === 1 ? "" : "s"}</p>
      </div>
      <button className="filmverket-secondary-action" onClick={onCopy} type="button">
        {copyState === "scene" ? "Scene copied" : copyState === "failed" ? "Copy failed" : "Copy active scene"}
      </button>
    </section>
  );
}
...
function ShotListEditor({ onAdd, onChange, onDelete, onDuplicate, onMove, scene }: {
  ...
  return (
    <section className="director-shot-list-section" id="director-shot-list">
      <header>
        <div><span className="filmverket-card-kicker">From scene strategy to coverage</span><h2>Shot cards</h2><p>Each card must explain both the production setup and the dramatic reason for the shot.</p></div>
        <button className="filmverket-primary-action" onClick={onAdd} type="button">+ Add shot</button>
      </header>
      {scene.shots.length === 0 ? (
        <div className="director-shot-empty">
          <strong>No shot cards yet.</strong>
          <p>Define the scene's coverage rule above, then add the first setup.</p>
          <button onClick={onAdd} type="button">Add first shot</button>
        </div>
      ) : (
...
function ShotCard({ index, onChange, onDelete, onDuplicate, onMove, shot, shotCount }: {
  ...
  const complete = countCompletedDirectorShotFields(shot);
  return (
    <article className="director-shot-card">
      <header>
        <span>{String(index + 1).padStart(2, "0")}</span>
        <div><strong>{shot.title.trim() || `Shot ${index + 1}`}</strong><small>{complete}/{DIRECTOR_SHOT_FIELDS.length} fields defined</small></div>
        <div className="director-shot-card-actions">
          <button aria-label="Move shot up" disabled={index === 0} onClick={() => onMove(shot.id, -1)} type="button">↑</button>
          <button aria-label="Move shot down" disabled={index === shotCount - 1} onClick={() => onMove(shot.id, 1)} type="button">↓</button>
          <button onClick={() => onDuplicate(shot.id)} type="button">Duplicate</button>
          <button onClick={() => onDelete(shot.id)} type="button">Delete</button>
        </div>
      </header>
      <div className="director-shot-grid">
        <ShotInput field="title" label="Shot title" onChange={onChange} shot={shot} />
        <label className="director-shot-field">
          <span>Shot size</span>
          <select onChange={(event) => onChange(shot.id, "shotSize", event.target.value)} value={shot.shotSize}>
            <option value="">Choose size…</option>
            {shotSizeOptions.map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
        </label>
        <ShotInput field="cameraPosition" label="Camera position" onChange={onChange} shot={shot} />
        <ShotInput field="movement" label="Movement" onChange={onChange} shot={shot} />
        <ShotInput field="lens" label="Lens / focal behavior" onChange={onChange} shot={shot} />
        <ShotInput field="estimatedDuration" label="Estimated duration" onChange={onChange} shot={shot} />
        <ShotTextArea field="subjectAction" label="Subject action and blocking" onChange={onChange} shot={shot} />
        <ShotTextArea field="dramaticPurpose" label="Dramatic purpose" onChange={onChange} shot={shot} />
        <ShotTextArea field="sound" label="Sound and dialogue priority" onChange={onChange} shot={shot} />
      </div>
    </article>
  );
}

function ShotInput({ field, label, onChange, shot }: {
  readonly field: DirectorShotFieldId;
  readonly label: string;
  readonly onChange: (shotId: string, field: DirectorShotFieldId, value: string) => void;
  readonly shot: DirectorShotCard;
}) {
...
function ShotTextArea({ field, label, onChange, shot }: {
  readonly field: DirectorShotFieldId;
  readonly label: string;
  readonly onChange: (shotId: string, field: DirectorShotFieldId, value: string) => void;
  readonly shot: DirectorShotCard;
...
function buildActiveSceneText(project: DirectorProject, activeScene: DirectorScene): string {
  return buildDirectorProjectText({
    ...project,
...
function loadProject(
  ...
  legacyBriefStorageKey: string,
  identity: { readonly filmId: string; readonly filmTitle: string; readonly filmYear: number },
): DirectorProject {
  try {
    const storedProject = window.localStorage.getItem(projectStorageKey);
    if (storedProject) {
      return coerceDirectorProject(JSON.parse(storedProject) as unknown, identity, createDirectorEntityId("scene"));
    }
    const storedBrief = window.localStorage.getItem(legacyBriefStorageKey);
    if (storedBrief) {
      const legacyBrief = coerceDirectorBriefDraft(JSON.parse(storedBrief) as unknown, identity);
      return createDirectorProjectFromBrief(legacyBrief, createDirectorEntityId("scene"));
    }
  } catch {
