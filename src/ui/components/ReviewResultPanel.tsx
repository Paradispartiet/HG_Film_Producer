...
export function ReviewResultPanel({ reviews, critics }: ReviewResultPanelProps) {
  ...
  const copy = STUDIO_CAREER_RELEASE_COPY[language].reviews;
  return (
    <ResultSection eyebrow={copy.eyebrow} title={copy.heading}>
        ...
        {reviews.map((review) => {
          ...
          return (
            <article className="release-result-card review-card" key={review.criticProfileId}>
              <div className="result-card-heading"><div><span>{copy.criticTypes[critic.type]}</span><strong>{critic.name}</strong></div><b>{review.score}</b></div>
              <blockquote>“{review.pullQuote}”</blockquote>
              <div className={`sentiment sentiment--${review.sentiment}`}>{copy.sentiments[review.sentiment]}</div>
              <dl className="release-delta-grid"><Metric label={copy.reputation} value={signed(review.reputationDelta)} /><Metric label={copy.prestige} value={signed(review.prestigeDelta)} /></dl>
            </article>
          );
        })}
      </div>
