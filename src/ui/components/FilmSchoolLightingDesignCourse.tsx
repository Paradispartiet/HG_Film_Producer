...
import { getLightingDesignLessonCopy } from "../../core/filmSchoolLightingDesignLessonCopy";
import {
...
export function FilmSchoolLightingDesignCourse({
  ...
  const surface = FILM_SCHOOL_COURSE_SURFACE_COPY[language].lightingDesign;
  ...
  const activeCopy = getLightingDesignLessonCopy(language, activeLesson);
  ...
  const remainingModules = LIGHTING_DESIGN_COURSE_LESSONS.length - masteredCount;
  ...
  return (
    <div className="filmverket-shell school-course-shell school-lighting-design-course-shell">
      ...
      <main className="film-school-course-page">
        <FilmSchoolCourseNavigation activeCourseId="lightingDesign" language={language} onSelectCourse={onSelectCourse} />

        <section className="school-course-hero school-course-hero--lighting-design">
          <div>
            <span className="filmverket-kicker">Film School · {chrome.foundation} · 4/5</span>
          ...
          <aside>
            ...
            <p>{masteredCount} / {LIGHTING_DESIGN_COURSE_LESSONS.length} {chrome.modules}</p>
          </aside>
        </section>

        <section className="school-course-summary">
          <div><strong>{LIGHTING_DESIGN_COURSE_LESSONS.length}</strong><span>{chrome.modules}</span></div>
          <div><strong>{new Set(LIGHTING_DESIGN_COURSE_LESSONS.flatMap((lesson) => lesson.termIds)).size}</strong><span>{chrome.terms}</span></div>
          <div><strong>{LIGHTING_DESIGN_COURSE_LESSONS.length}</strong><span>{chrome.filmExamples}</span></div>
        ...
        <section className="school-course-workspace">
          <aside className="school-lesson-rail">
            ...
            <div>{LIGHTING_DESIGN_COURSE_LESSONS.map((lesson) => { const lessonStage = getLightingDesignLessonMasteryStage(progress, lesson.id); const lessonCopy = getLightingDesignLessonCopy(language, lesson); return <button className={lesson.id === activeLesson.id ? "school-lesson-button is-active" : "school-lesson-button"} key={lesson.id} onClick={() => selectLesson(lesson.id)} type="button"><span>{lesson.number}</span><div><strong>{lessonCopy.title}</strong><small className={`stage-${lessonStage}`}>{stageLabels[lessonStage]}</small></div></button>; })}</div>
          </aside>
          ...
          <article className="school-active-lesson" id="school-active-lighting-design-lesson">
            ...
            <section className="school-quiz-card">
              ...
              <div>{activeCopy.quiz.options.map((option, optionIndex) => { const selected = selectedAnswer === optionIndex; const correct = selected && isLightingDesignCourseQuizAnswerCorrect(activeLesson, optionIndex); return <button className={selected ? correct ? "is-selected is-correct" : "is-selected is-wrong" : ""} key={`${optionIndex}:${option}`} onClick={() => answerQuiz(activeLesson, optionIndex)} type="button"><span>{String.fromCharCode(65 + optionIndex)}</span>{option}</button>; })}</div>
            ...
            <nav className="school-lesson-navigation" aria-label={chrome.courseModuleNavAria}>
              <button disabled={activeIndex <= 0} onClick={() => { const previous = LIGHTING_DESIGN_COURSE_LESSONS[activeIndex - 1]; if (previous) selectLesson(previous.id); }} type="button">{chrome.previousModule}</button>
              <button disabled={activeIndex >= LIGHTING_DESIGN_COURSE_LESSONS.length - 1} onClick={() => { const next = LIGHTING_DESIGN_COURSE_LESSONS[activeIndex + 1]; if (next) selectLesson(next.id); }} type="button">{chrome.nextModule}</button>
            </nav>
          </article>
        </section>
