...
export const careerRunActions = {
  ...
  applyCareerResult(state: CareerRunState, projectId: string): CareerRunState { return updateProject(state, projectId, (project) => project.releaseResult ? { ...project, careerApplicationResult: createCareerApplicationStepResult(getRunContext(project.run), project.releaseResult, { closeFilmYear: true }) } : project); },
