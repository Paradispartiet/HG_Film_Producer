...
import { ordetFilmHistoryProfile } from "./scenarioFilmStudyPostwarEuropeanOrdet";
import { ashesAndDiamondsFilmHistoryProfile } from "./scenarioFilmStudyPostwarEuropeanAshesAndDiamonds";
import { personaFilmHistoryProfile } from "./scenarioFilmStudyPostwarEuropeanPersona";
import { daisiesFilmHistoryProfile } from "./scenarioFilmStudyPostwarEuropeanDaisies";

const profiles = {
  [ordetFilmHistoryProfile.scenarioId]: ordetFilmHistoryProfile,
  [ashesAndDiamondsFilmHistoryProfile.scenarioId]: ashesAndDiamondsFilmHistoryProfile,
  [personaFilmHistoryProfile.scenarioId]: personaFilmHistoryProfile,
  [daisiesFilmHistoryProfile.scenarioId]: daisiesFilmHistoryProfile,
...
export function getPostwarEuropeanModernismFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolvePostwarEuropeanModernismFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getPostwarEuropeanModernismFilmHistoryProfile(scenario.id);
...
export function createPostwarEuropeanModernismFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between postwar history, production organization and the film's formal system.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is another real postwar European modernist system, but it organizes duration, performance, politics, image and editing differently.",
    }] : []),
    ...(far ? [{
