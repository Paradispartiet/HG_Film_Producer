...
test("createScenarioPhaseGuidance maps brief targets to all production phases", () => {
  ...
  assert.deepEqual(guidance.map((phase) => phase.phase), ["development", "pre_production", "shoot", "post_production", "release"]);
  assert.deepEqual(guidance.find((phase) => phase.phase === "development")?.targets, ["Genre A", "Tone A", "Tone B", "Screenplay A"]);
  assert.deepEqual(guidance.find((phase) => phase.phase === "shoot")?.targets, ["Cinematography A", "Sound A", "Sound B"]);
  assert.deepEqual(guidance.find((phase) => phase.phase === "post_production")?.targets, ["Editing A", "Sound A", "Sound B"]);
  assert.deepEqual(guidance.find((phase) => phase.phase === "release")?.targets, ["Learning A"]);
});
