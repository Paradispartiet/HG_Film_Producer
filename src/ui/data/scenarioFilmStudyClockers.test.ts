...
import { getClockersDonorScenarioIds } from "./scenarioFilmStudyCrimeNoirClockersCatalog.js";
import {
...
const scenarioId = "scenario_clockers_1995";

test("Clockers resolves as a source-backed crime-noir transformation system", () => {
...
test("Clockers receives dedicated donors without entering existing crime-noir choices", () => {
  const profile = getCrimeNoirTransformationsFilmHistoryProfile(scenarioId);
  assert.ok(profile);

  assert.deepEqual(
    getClockersDonorScenarioIds(profile),
    [
  ...
  for (const existingScenarioId of [
    ...
    assert.equal(existingChoices.some((choice) => choice.label.includes("Gowanus")), false);
    assert.equal(existingChoices.some((choice) => choice.label.includes("cross-processed Ektachrome")), false);
  }
});
