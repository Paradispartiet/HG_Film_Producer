...
import { closelyWatchedTrainsFilmHistoryProfile } from "./scenarioFilmStudyCzechoslovakCloselyWatchedTrains";
import { crematorFilmHistoryProfile } from "./scenarioFilmStudyCzechoslovakCremator";
import { firemensBallFilmHistoryProfile } from "./scenarioFilmStudyCzechoslovakFiremensBall";
import { marketaLazarovaFilmHistoryProfile } from "./scenarioFilmStudyCzechoslovakMarketaLazarova";

const profiles = {
  [closelyWatchedTrainsFilmHistoryProfile.scenarioId]: closelyWatchedTrainsFilmHistoryProfile,
  [firemensBallFilmHistoryProfile.scenarioId]: firemensBallFilmHistoryProfile,
  [marketaLazarovaFilmHistoryProfile.scenarioId]: marketaLazarovaFilmHistoryProfile,
  [crematorFilmHistoryProfile.scenarioId]: crematorFilmHistoryProfile,
...
export function getCzechoslovakNewWaveFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolveCzechoslovakNewWaveFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getCzechoslovakNewWaveFilmHistoryProfile(scenario.id);
...
export function createCzechoslovakNewWaveFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between Czechoslovak production conditions and the film's formal system.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real Czechoslovak 1960s production system, but it organizes performance, space, history, editing and sound differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong Czechoslovak production tradition and craft logic.",
    }] : []),
  ];
}
