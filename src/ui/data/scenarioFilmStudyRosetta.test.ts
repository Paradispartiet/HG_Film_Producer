...
const scenarioId = "scenario_rosetta_1999";

test("Rosetta resolves as a source-backed embodied labour realism system", () => {
...
test("Rosetta receives dedicated social-realism donors without changing existing choices", () => {
  ...
  for (const existingScenarioId of [
    ...
    assert.equal(existingChoices.some((choice) => choice.label.includes("gas bottles")), false);
    assert.equal(existingChoices.some((choice) => choice.label.includes("waffle stand")), false);
  }
});
