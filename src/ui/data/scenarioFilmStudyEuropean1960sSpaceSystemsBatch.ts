...
import { lastYearAtMarienbadFilmHistoryProfile } from "./scenarioFilmStudyEuropean1960sSpaceMarienbad";
import { playtimeFilmHistoryProfile } from "./scenarioFilmStudyEuropean1960sSpacePlaytime";
import { theLeopardFilmHistoryProfile } from "./scenarioFilmStudyEuropean1960sSpaceLeopard";

const profiles = {
  [lAvventuraFilmHistoryProfile.scenarioId]: lAvventuraFilmHistoryProfile,
  [lastYearAtMarienbadFilmHistoryProfile.scenarioId]: lastYearAtMarienbadFilmHistoryProfile,
  [playtimeFilmHistoryProfile.scenarioId]: playtimeFilmHistoryProfile,
  [theLeopardFilmHistoryProfile.scenarioId]: theLeopardFilmHistoryProfile,
...
export function getEuropean1960sSpaceSystemsFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolveEuropean1960sSpaceSystemsFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getEuropean1960sSpaceSystemsFilmHistoryProfile(scenario.id);
...
export function createEuropean1960sSpaceSystemsFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between 1960s European production scale, architecture, landscape, duration, performance, image, editing, music and sound.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real 1960s European space-production system, but it organises landscape absence, architectural memory, built-city comedy and historical widescreen ceremony differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong spatial, industrial and temporal production logic.",
    }] : []),
  ];
}
