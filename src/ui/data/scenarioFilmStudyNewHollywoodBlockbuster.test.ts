...
test("New Hollywood and blockbuster batch resolves complete source-backed maps", () => {
  const scenarios = new Map(getClassicFilmScenarios().map((scenario) => [scenario.id, scenario]));
  for (const scenarioId of scenarioIds) {
    const scenario = scenarios.get(scenarioId);
    assert.ok(scenario, `Missing scenario ${scenarioId}`);
    const brief = resolveScenarioProductionBrief(scenario);
    const study = resolveNewHollywoodBlockbusterFilmStudyMap(scenario, brief);
...
test("New Hollywood and blockbuster choices compare three real systems", () => {
  for (const scenarioId of scenarioIds) {
