...
import { rashomonFilmHistoryProfile } from "./scenarioFilmStudy1950sRashomon";
import { tokyoStoryFilmHistoryProfile } from "./scenarioFilmStudy1950sTokyoStory";
import { sevenSamuraiFilmHistoryProfile } from "./scenarioFilmStudy1950sSevenSamurai";
import { patherPanchaliFilmHistoryProfile } from "./scenarioFilmStudy1950sPatherPanchali";

const profiles = {
  [rashomonFilmHistoryProfile.scenarioId]: rashomonFilmHistoryProfile,
  [tokyoStoryFilmHistoryProfile.scenarioId]: tokyoStoryFilmHistoryProfile,
  [sevenSamuraiFilmHistoryProfile.scenarioId]: sevenSamuraiFilmHistoryProfile,
  [patherPanchaliFilmHistoryProfile.scenarioId]: patherPanchaliFilmHistoryProfile,
...
export function get1950sAsianPostwarFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolve1950sAsianPostwarFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = get1950sAsianPostwarFilmHistoryProfile(scenario.id);
...
export function create1950sAsianPostwarFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between postwar context, production organization and the film's formal system.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real 1950s Asian production system, but it organizes performance, space, duration, action and realism differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong postwar production tradition and craft logic.",
    }] : []),
  ];
}
