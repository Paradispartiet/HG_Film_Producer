...
import { laStradaFilmHistoryProfile } from "./scenarioFilmStudy1950sExpressiveLaStrada";
import { nightOfTheHunterFilmHistoryProfile } from "./scenarioFilmStudy1950sExpressiveNightOfTheHunter";
import { seventhSealFilmHistoryProfile } from "./scenarioFilmStudy1950sExpressiveSeventhSeal";
import { touchOfEvilFilmHistoryProfile } from "./scenarioFilmStudy1950sExpressiveTouchOfEvil";

const profiles = {
  [laStradaFilmHistoryProfile.scenarioId]: laStradaFilmHistoryProfile,
  [nightOfTheHunterFilmHistoryProfile.scenarioId]: nightOfTheHunterFilmHistoryProfile,
  [seventhSealFilmHistoryProfile.scenarioId]: seventhSealFilmHistoryProfile,
  [touchOfEvilFilmHistoryProfile.scenarioId]: touchOfEvilFilmHistoryProfile,
...
export function get1950sExpressivePostwarFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolve1950sExpressivePostwarFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = get1950sExpressivePostwarFilmHistoryProfile(scenario.id);
...
export function create1950sExpressivePostwarFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between the film's postwar production conditions, expressive design and specific craft system.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real 1950s expressive production system, but it organizes location, performance, design, camera, editing and sound differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong national, industrial and stylistic production logic.",
    }] : []),
  ];
}
