...
test("Hong Kong and Taiwan urban-time batch resolves complete source-backed maps", () => {
...
test("Hong Kong and Taiwan urban-time choices compare three real systems", () => {
  for (const scenarioId of scenarioIds) {
    ...
    assert.ok(choices.slice(1).every((choice) => (
      choice.label.includes("Hong Kong") || choice.label.includes("Taiwan") || choice.label.includes("Taiwanese")
    )));
  }
});
