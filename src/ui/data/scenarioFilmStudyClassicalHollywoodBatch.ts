...
import { stagecoachFilmHistoryProfile } from "./scenarioFilmStudyClassicalHollywoodStagecoach";
import { wizardOfOzFilmHistoryProfile } from "./scenarioFilmStudyClassicalHollywoodWizardOfOz";
import { citizenKaneFilmHistoryProfile } from "./scenarioFilmStudyClassicalHollywoodCitizenKane";
import { casablancaFilmHistoryProfile } from "./scenarioFilmStudyClassicalHollywoodCasablanca";

const profiles = {
  [stagecoachFilmHistoryProfile.scenarioId]: stagecoachFilmHistoryProfile,
  [wizardOfOzFilmHistoryProfile.scenarioId]: wizardOfOzFilmHistoryProfile,
  [citizenKaneFilmHistoryProfile.scenarioId]: citizenKaneFilmHistoryProfile,
  [casablancaFilmHistoryProfile.scenarioId]: casablancaFilmHistoryProfile,
...
export function getClassicalHollywoodFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolveClassicalHollywoodFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getClassicalHollywoodFilmHistoryProfile(scenario.id);
...
export function createClassicalHollywoodFilmHistoryChoices(
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between studio organization, historical context and the film's formal system.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real classical Hollywood system, but its departments solve scale, performance, color, space and sound differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong studio tradition and production logic.",
    }] : []),
  ];
}
