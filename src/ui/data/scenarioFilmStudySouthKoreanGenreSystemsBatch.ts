...
export function getSouthKoreanGenreSystemsFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolveSouthKoreanGenreSystemsFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getSouthKoreanGenreSystemsFilmHistoryProfile(scenario.id);
...
export function createSouthKoreanGenreSystemsFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between South Korean historical context, production organization, genre system and film-specific craft.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real South Korean production system, but it organizes history, performance, location, genre, effects, editing and sound differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong South Korean production tradition and craft logic.",
    }] : []),
  ];
}
