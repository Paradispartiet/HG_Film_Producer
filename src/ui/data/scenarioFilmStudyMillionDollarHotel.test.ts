...
import { getTheMillionDollarHotelFilmHistoryDonors } from "./scenarioFilmStudyOutsiderHotelMusicMillionDollarHotelCatalog.js";
import {
...
const scenarioId = "scenario_the_million_dollar_hotel_2000";

test("The Million Dollar Hotel resolves as a source-backed outsider hotel and music system", () => {
...
test("The Million Dollar Hotel receives dedicated hotel, ensemble and music donors without changing existing choices", () => {
  const profile = getIndependentStorytellingFilmHistoryProfile(scenarioId);
  assert.ok(profile);

  assert.deepEqual(
    getTheMillionDollarHotelFilmHistoryDonors(profile)?.map((donor) => donor.scenarioId),
    [
  ...
  for (const existingScenarioId of [
    ...
    assert.equal(existingChoices.some((choice) => choice.label.includes("Bono-Klein")), false);
    assert.equal(existingChoices.some((choice) => choice.label.includes("Million Dollar Hotel")), false);
  }
});
