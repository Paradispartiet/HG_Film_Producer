...
import { aguirreFilmHistoryProfile } from "./scenarioFilmStudyNewGermanCinemaAguirre";
import { aliFearEatsTheSoulFilmHistoryProfile } from "./scenarioFilmStudyNewGermanCinemaAli";
import { marriageOfMariaBraunFilmHistoryProfile } from "./scenarioFilmStudyNewGermanCinemaMariaBraun";
import { wingsOfDesireFilmHistoryProfile } from "./scenarioFilmStudyNewGermanCinemaWingsOfDesire";

const profiles = {
  [aguirreFilmHistoryProfile.scenarioId]: aguirreFilmHistoryProfile,
  [aliFearEatsTheSoulFilmHistoryProfile.scenarioId]: aliFearEatsTheSoulFilmHistoryProfile,
  [marriageOfMariaBraunFilmHistoryProfile.scenarioId]: marriageOfMariaBraunFilmHistoryProfile,
  [wingsOfDesireFilmHistoryProfile.scenarioId]: wingsOfDesireFilmHistoryProfile,
...
export function getNewGermanCinemaSystemsFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolveNewGermanCinemaSystemsFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getNewGermanCinemaSystemsFilmHistoryProfile(scenario.id);
...
export function createNewGermanCinemaSystemsFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between New German Cinema, production conditions, national history, performance, location, image, editing, music and sound.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real New German Cinema production system, but it organises extreme location, social melodrama, postwar allegory and poetic city listening differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong historical, industrial and audiovisual production logic.",
    }] : []),
  ];
}
