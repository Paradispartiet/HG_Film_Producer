...
import { milkOfSorrowFilmHistoryProfile } from "./scenarioFilmStudyIntimateFestivalMilkOfSorrow";
import { somewhereFilmHistoryProfile } from "./scenarioFilmStudyIntimateFestivalSomewhere";
import { veraDrakeFilmHistoryProfile } from "./scenarioFilmStudyIntimateFestivalVeraDrake";
import { wrestlerFilmHistoryProfile } from "./scenarioFilmStudyIntimateFestivalWrestler";

const profiles = {
  [veraDrakeFilmHistoryProfile.scenarioId]: veraDrakeFilmHistoryProfile,
  [wrestlerFilmHistoryProfile.scenarioId]: wrestlerFilmHistoryProfile,
  [milkOfSorrowFilmHistoryProfile.scenarioId]: milkOfSorrowFilmHistoryProfile,
  [somewhereFilmHistoryProfile.scenarioId]: somewhereFilmHistoryProfile,
...
export function getIntimateFestivalBodyCareSystemsFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolveIntimateFestivalBodyCareSystemsFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getIntimateFestivalBodyCareSystemsFilmHistoryProfile(scenario.id);
...
export function createIntimateFestivalBodyCareSystemsFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between intimate social pressure, body or care, festival-era production, performance, image, editing and sound.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real intimate festival production system, but it organizes research, body, labour, trauma, celebrity, duration and care differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong historical, industrial and formal body-and-care production logic.",
    }] : []),
  ];
}
