...
import { hiroshimaMonAmourFilmHistoryProfile } from "./scenarioFilmStudyEuropeanModernistHiroshimaMonAmour";
import { umbrellasOfCherbourgFilmHistoryProfile } from "./scenarioFilmStudyEuropeanModernistUmbrellasOfCherbourg";

const profiles = {
  [hiroshimaMonAmourFilmHistoryProfile.scenarioId]: hiroshimaMonAmourFilmHistoryProfile,
  [eightAndAHalfFilmHistoryProfile.scenarioId]: eightAndAHalfFilmHistoryProfile,
  [umbrellasOfCherbourgFilmHistoryProfile.scenarioId]: umbrellasOfCherbourgFilmHistoryProfile,
  [battleOfAlgiersFilmHistoryProfile.scenarioId]: battleOfAlgiersFilmHistoryProfile,
...
export function getEuropeanModernistProductionFilmHistoryProfile(scenarioId: string): FilmHistoryProfile | undefined {
  return profiles[scenarioId as keyof typeof profiles];
}

export function resolveEuropeanModernistProductionFilmStudyMap(
  scenario: FilmScenarioSeed,
  brief: ScenarioProductionBrief,
): ScenarioFilmStudyMap | undefined {
  const historyProfile = getEuropeanModernistProductionFilmHistoryProfile(scenario.id);
...
export function createEuropeanModernistProductionFilmHistoryChoices(profile: FilmHistoryProfile): readonly FilmHistoryChoice[] {
  ...
  return [
    {
      ...
      quality: "match",
      feedback: "This matches the documented relationship between European modernism and the film's specific production, image, editing and sound system.",
    },
    ...(near ? [{
      id: `${profile.scenarioId}-history-partial`,
      label: `${near.period}: ${near.moment}`,
      quality: "partial" as const,
      feedback: "This is a real European modernist production system, but it organizes history, performance, design, camera, editing and sound differently.",
    }] : []),
    ...(far ? [{
      id: `${profile.scenarioId}-history-miss`,
      label: `${far.period}: ${far.moment}`,
      quality: "miss" as const,
      feedback: "This assigns the film to the wrong modernist production method and historical relationship between evidence, artifice and reconstruction.",
    }] : []),
  ];
}
