...
function CareerDashboard({ careerRun, setCareerRun, onOpenProductionCases, onResetCareer, onStartScenario }: { readonly careerRun: readonly CareerFilmProjectRun[]; readonly setCareerRun: React.Dispatch<React.SetStateAction<{ version: 1; projects: readonly CareerFilmProjectRun[] }>>; readonly onOpenProductionCases: () => void; readonly onResetCareer: () => void; readonly onStartScenario: (scenario: FilmScenarioSeed) => void; }) {
  ...
  const latestProject = careerRun[careerRun.length - 1];
  const isProductionCaseRun = Boolean(latestProject && "classicScenarioId" in latestProject.run && latestProject.run.classicScenarioId);
...
function FilmProjectWorkspace({ project, isLatest, onOpenProductionCases, onStartScenario, previousProject, setCareerRun }: { readonly project: CareerFilmProjectRun; readonly isLatest: boolean; readonly onOpenProductionCases: () => void; readonly onStartScenario: (scenario: FilmScenarioSeed) => void; readonly previousProject: CareerFilmProjectRun | undefined; readonly setCareerRun: React.Dispatch<React.SetStateAction<{ version: 1; projects: readonly CareerFilmProjectRun[] }>>; }) {
  const [language] = useFilmWorkLanguage();
  const workspaceCopy = STUDIO_CAREER_WORKSPACE_COPY[language];
  const [manualExpand, setManualExpand] = useState<boolean | null>(null);
  const [expandedPhaseIds, setExpandedPhaseIds] = useState<readonly number[]>([]);
  const phaseExpanded = (phase: number) => expandedPhaseIds.includes(phase);
  const togglePhase = (phase: number) => setExpandedPhaseIds((current) => current.includes(phase) ? current.filter((candidate) => candidate !== phase) : [...current, phase]);
  ...
  return <section className={collapsed ? "panel next-project-panel next-project-panel--collapsed" : "panel next-project-panel"}>{!isLatest && pastSummary && <div className="past-project-summary"><div><span className="eyebrow">{workspaceCopy.workspace.completeSummary(project.projectNumber)}</span><h3>{project.run.project.title}</h3><p>{pastSummary.detail}</p></div><div className="past-project-summary-actions"><strong>{pastSummary.score}</strong><button className="secondary-button" onClick={() => setManualExpand((current) => current === null ? true : !current)} type="button">{collapsed ? workspaceCopy.completed.showDetails : workspaceCopy.completed.hideDetails}</button></div></div>}{!collapsed && <><ProjectPipeline currentPhase={currentPhase} isProductionCase={Boolean(classicScenario)} onNextAction={scrollToActivePanel} project={project.run.project} steps={pipeline} />{classicScenario && <ScenarioProductionBriefPanel onBackToProductionCases={onOpenProductionCases} onStartNextScenario={nextClassicScenario ? () => onStartScenario(nextClassicScenario) : undefined} scenario={classicScenario} />}{classicScenarioBrief && <><ScenarioTargetChecklistPanel brief={classicScenarioBrief} selectedTargetIds={project.selectedScenarioTargetIds ?? []} onChangeSelectedTargetIds={(targetIds) => setCareerRun((state) => careerRunActions.changeScenarioTargetSelections(state, project.id, targetIds))} /><ScenarioAlignmentScorePanel selectedTargetIds={project.selectedScenarioTargetIds ?? []} totalTargets={classicScenarioTargetCount} /><ScenarioPhaseGuidancePanel brief={classicScenarioBrief} currentPhase={currentPhase} /></>}{previousProject?.careerApplicationResult && <div className="next-project-carryover-grid"><StudioCarryoverPanel careerState={previousProject.careerApplicationResult.updatedCareerState} sourceFilmLabel={`film ${previousProject.projectNumber}`} /></div>}{project.projectNumber > 1 && <NextProjectResultPanel careerApplicationResult={project.careerApplicationResult} developmentResult={project.developmentResult} postProductionResult={project.postProductionResult} preProductionResult={project.preProductionResult} releaseResult={project.releaseResult} result={project.run as never} shootResult={project.shootResult} projectNumber={project.projectNumber} />}{project.developmentResult ? <><CompletedPhaseRow expanded={phaseExpanded(0)} onToggle={() => togglePhase(0)} phaseLabel={workspaceCopy.phases.development.label} step={project.developmentResult.pipelineStep} />{phaseExpanded(0) && <DevelopmentResultPanel projectLabel={label} results={project.developmentResults} />}{project.preProductionResult ? <><CompletedPhaseRow expanded={phaseExpanded(1)} onToggle={() => togglePhase(1)} phaseLabel={workspaceCopy.phases.preProduction.label} step={project.preProductionResult.pipelineStep} />{phaseExpanded(1) && <ProductionTeamResultPanel compact={project.projectNumber === 1} projectLabel={label} result={project.preProductionResult} />}{project.shootResult ? <><CompletedPhaseRow expanded={phaseExpanded(2)} onToggle={() => togglePhase(2)} phaseLabel={workspaceCopy.phases.shoot.label} step={project.shootResult.pipelineStep} />{phaseExpanded(2) && <ShootPanel developmentResult={project.developmentResult} id={activePanelTarget === "shoot" ? actionPanelId : undefined} onResolveShootDay={(result) => setCareerRun((state) => careerRunActions.resolveShootDay(state, project.id, result))} onSelectProductionEvent={(eventId) => setCareerRun((state) => careerRunActions.selectProductionEvent(state, project.id, eventId))} preProductionResult={project.preProductionResult} projectContext={context} projectLabel={lowerLabel} selectedProductionEventId={project.selectedProductionEventId} shootDayResults={project.shootDayResults} shootResult={project.shootResult} />}{project.postProductionResult ? <><CompletedPhaseRow expanded={phaseExpanded(3)} onToggle={() => togglePhase(3)} phaseLabel={workspaceCopy.phases.postProduction.label} step={project.postProductionResult.pipelineStep} />{phaseExpanded(3) && <PostProductionPanel choices={project.postProductionChoices} id={activePanelTarget === "post-production" ? actionPanelId : undefined} onChange={(choices) => setCareerRun((state) => careerRunActions.changePostProductionChoices(state, project.id, choices))} onLock={(result) => setCareerRun((state) => careerRunActions.lockPostProduction(state, project.id, result))} preProductionResult={project.preProductionResult} projectContext={context} projectLabel={lowerLabel} result={project.postProductionResult} shootResult={project.shootResult} />}{project.releaseResult ? <>{classicScenario && classicScenarioBrief && <><ScenarioAlignmentResultPanel selectedTargetIds={project.selectedScenarioTargetIds ?? []} totalTargets={classicScenarioTargetCount} /><ScenarioLearningRecapPanel brief={classicScenarioBrief} scenarioTitle={classicScenario.film.title} selectedTargetIds={project.selectedScenarioTargetIds ?? []} /></>}<CompletedPhaseRow expanded={phaseExpanded(4)} onToggle={() => togglePhase(4)} phaseLabel={workspaceCopy.phases.release.label} step={project.releaseResult.pipelineStep} />{phaseExpanded(4) && <ReleaseStepPanel choices={project.releaseChoices} developmentResult={project.developmentResult} id={activePanelTarget === "release" ? actionPanelId : undefined} onChange={(choices) => setCareerRun((state) => careerRunActions.changeReleaseChoices(state, project.id, choices))} onRelease={(result) => setCareerRun((state) => careerRunActions.releaseFilm(state, project.id, result))} postProductionResult={project.postProductionResult} preProductionResult={project.preProductionResult} projectContext={context} projectLabel={lowerLabel} result={project.releaseResult} shootResult={project.shootResult} />}{project.careerApplicationResult ? <><CompletedPhaseRow expanded={phaseExpanded(5)} onToggle={() => togglePhase(5)} phaseLabel={workspaceCopy.phases.careerReview.label} step={project.careerApplicationResult.pipelineStep} />{phaseExpanded(5) && <CareerApplicationPanel id={activePanelTarget === "career-application" ? actionPanelId : undefined} onApply={() => setCareerRun((state) => careerRunActions.applyCareerResult(state, project.id))} projectContext={context} projectLabel={lowerLabel} releaseResult={project.releaseResult} result={project.careerApplicationResult} strategicGoal={strategicGoal} />}</> : <CareerApplicationPanel id={activePanelTarget === "career-application" ? actionPanelId : undefined} onApply={() => setCareerRun((state) => careerRunActions.applyCareerResult(state, project.id))} projectContext={context} projectLabel={lowerLabel} releaseResult={project.releaseResult} result={project.careerApplicationResult} strategicGoal={strategicGoal} />}</> : <ReleaseStepPanel choices={project.releaseChoices} developmentResult={project.developmentResult} id={activePanelTarget === "release" ? actionPanelId : undefined} onChange={(choices) => setCareerRun((state) => careerRunActions.changeReleaseChoices(state, project.id, choices))} onRelease={(result) => setCareerRun((state) => careerRunActions.releaseFilm(state, project.id, result))} postProductionResult={project.postProductionResult} preProductionResult={project.preProductionResult} projectContext={context} projectLabel={lowerLabel} result={project.releaseResult} shootResult={project.shootResult} />}</> : <PostProductionPanel choices={project.postProductionChoices} id={activePanelTarget === "post-production" ? actionPanelId : undefined} onChange={(choices) => setCareerRun((state) => careerRunActions.changePostProductionChoices(state, project.id, choices))} onLock={(result) => setCareerRun((state) => careerRunActions.lockPostProduction(state, project.id, result))} preProductionResult={project.preProductionResult} projectContext={context} projectLabel={lowerLabel} result={project.postProductionResult} shootResult={project.shootResult} />}</> : <ShootPanel developmentResult={project.developmentResult} id={activePanelTarget === "shoot" ? actionPanelId : undefined} onResolveShootDay={(result) => setCareerRun((state) => careerRunActions.resolveShootDay(state, project.id, result))} onSelectProductionEvent={(eventId) => setCareerRun((state) => careerRunActions.selectProductionEvent(state, project.id, eventId))} preProductionResult={project.preProductionResult} projectContext={context} projectLabel={lowerLabel} selectedProductionEventId={project.selectedProductionEventId} shootDayResults={project.shootDayResults} shootResult={project.shootResult} />}</> : <PreProductionPanel developmentResult={project.developmentResult} id={activePanelTarget === "pre-production" ? actionPanelId : undefined} onLock={(result) => setCareerRun((state) => careerRunActions.lockPreProduction(state, project.id, result))} onSelectActors={(selectedActorIds) => setCareerRun((state) => careerRunActions.changePreProductionSelections(state, project.id, { ...project.preProductionSelections, selectedActorIds }))} onSelectCrew={(selectedCrewIds) => setCareerRun((state) => careerRunActions.changePreProductionSelections(state, project.id, { ...project.preProductionSelections, selectedCrewIds }))} onSelectLocation={(selectedLocationId) => setCareerRun((state) => careerRunActions.changePreProductionSelections(state, project.id, { ...project.preProductionSelections, selectedLocationId }))} projectContext={context} projectLabel={lowerLabel} selectedActorIds={project.preProductionSelections.selectedActorIds} selectedCrewIds={project.preProductionSelections.selectedCrewIds} selectedLocationId={project.preProductionSelections.selectedLocationId} />}</> : <DevelopmentPanel completedResults={project.developmentResults} id={activePanelTarget === "development" ? actionPanelId : undefined} onApplyAction={(result) => setCareerRun((state) => careerRunActions.applyDevelopmentAction(state, project.id, result))} onFinishDevelopment={() => setCareerRun((state) => careerRunActions.finishDevelopment(state, project.id))} onSelectPath={(path) => setCareerRun((state) => careerRunActions.selectDevelopmentPath(state, project.id, path))} projectContext={context} projectLabel={label} selectedPath={project.selectedDevelopmentPath} />}{project.careerApplicationResult && <NextProjectCreator previousProject={project} setCareerRun={setCareerRun} />}</>}</section>;
}
...
function NextProjectCreator({ previousProject, setCareerRun }: { readonly previousProject: CareerFilmProjectRun; readonly setCareerRun: React.Dispatch<React.SetStateAction<{ version: 1; projects: readonly CareerFilmProjectRun[] }>>; }) {
  ...
  function createProject() {
    ...
    const nextPanelId = getStudioCareerActivePanelId(createCareerProject(run, previousProject.projectNumber + 1).id);
