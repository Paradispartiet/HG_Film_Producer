...
WORKFLOW = Path('.github/workflows/nollywood-representation-admission.yml')
workflow = WORKFLOW.read_text()


def extract_cat(target: str) -> str:
    pattern = rf"cat > {re.escape(target)} <<'EOF'\n(.*?)\n\s*EOF"
    match = re.search(pattern, workflow, flags=re.S)
    if not match:
        raise SystemExit(f'missing canonical heredoc for {target}')
    return textwrap.dedent(match.group(1)) + '\n'


# Reuse the already-reviewed scenario and PV definitions verbatim from the admission workflow.
for target in [
    ...
    path.write_text(extract_cat(target))

# Reuse the admission workflow's canonical patch logic verbatim.
blocks = [textwrap.dedent(block) for block in re.findall(r"python3 - <<'PY'\n(.*?)\n\s*PY", workflow, flags=re.S)]
materializer = next((block for block in blocks if '# Runtime Atlas:' in block and '# Representation audit deliberately graduates' in block), None)
if materializer is None:
    raise SystemExit('canonical admission materializer block not found')
exec(compile(materializer, '<nollywood-admission-materializer>', 'exec'), {})

# Add the source-backed Film Study profile required by the production-case structural audit.
film_study = '''import type { FilmHistoryProfile } from "./scenarioFilmStudyMap";
...
'''
Path('src/ui/data/scenarioFilmStudyRepresentationLivingInBondage.ts').write_text(film_study)

map_path = Path('src/ui/data/scenarioFilmStudyMap.ts')
...
if map_text.count(import_anchor) != 1 or map_text.count(registry_anchor) != 1:
    raise SystemExit('Film Study map anchors drifted')
map_text = map_text.replace(import_anchor, import_anchor + 'import { livingInBondageFilmHistoryProfile } from "./scenarioFilmStudyRepresentationLivingInBondage";\n', 1)
map_text = map_text.replace(registry_anchor, registry_anchor + '  [livingInBondageFilmHistoryProfile.scenarioId]: livingInBondageFilmHistoryProfile,\n', 1)
map_path.write_text(map_text)

coverage_path = Path('data/film/film_history_representation_coverage_v1.json')
coverage = json.loads(coverage_path.read_text())
review = next((r for r in coverage['scenario_reviews'] if r.get('scenario_id') == 'scenario_living_in_bondage_1992'), None)
if review is None:
    raise SystemExit('Living in Bondage review missing after canonical materializer')
...
print('Nollywood admission materialized with source-backed Film Study profile')
